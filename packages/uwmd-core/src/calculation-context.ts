...
export function parseCalculationContext(value: unknown): ContextInputs {
  ...
  for (const key of Object.keys(input)) {
    if (key !== 'sectionVariants' && key !== 'overrides') {
      throw new CalcError(
        'CALC-TYPE-001',
        `Unknown calculation context option '${key}'. Expected sectionVariants or overrides.`
      );
    }
  }
  ...
  if (Object.hasOwn(input, 'sectionVariants')) {
    ...
    for (const [section, variant] of Object.entries(entries)) {
      ...
      if (typeof variant !== 'string' || !variant.trim()) {
        throw new CalcError(
          'CALC-TYPE-001',
          `sectionVariants.${section} must be a nonempty string.`
        );
      }
  ...
  if (Object.hasOwn(input, 'overrides')) {
    ...
    for (const [path, replacement] of Object.entries(entries)) {
      if (
        ...
        throw new CalcError(
          'CALC-TYPE-001',
          `Override '${path}' requires a nonempty exact path and a finite JSON scalar or null.`
        );
      }
