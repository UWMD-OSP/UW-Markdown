...
function inspectZip(input: Uint8Array, options: UWCSVDecodeOptions): void {
  ...
  if (input.byteLength > maxCompressed) throw new UWCSVError('CSV_ZIP_SIZE_LIMIT', `ZIP is ${input.byteLength} bytes; limit is ${maxCompressed}.`);
  const view = new DataView(input.buffer, input.byteOffset, input.byteLength);
  let eocd = -1;
  for (let index = input.byteLength - 22; index >= Math.max(0, input.byteLength - 65_557); index--) {
    if (view.getUint32(index, true) === 0x06054b50) { eocd = index; break; }
  }
  if (eocd < 0) throw new UWCSVError('CSV_ZIP_INVALID', 'ZIP end record is missing.');
  ...
  const maxFiles = options.maxFiles ?? MAX_FILES;
  if (count > maxFiles) throw new UWCSVError('CSV_FILE_LIMIT', `ZIP contains ${count} files; limit is ${maxFiles}.`);
  if (centralOffset + centralSize > eocd) throw new UWCSVError('CSV_ZIP_INVALID', 'ZIP central directory is outside the archive.');
  ...
  for (let index = 0; index < count; index++) {
    if (offset + 46 > input.byteLength || view.getUint32(offset, true) !== 0x02014b50) throw new UWCSVError('CSV_ZIP_INVALID', 'ZIP central entry is invalid.');
    ...
    if ((flags & 1) !== 0) throw new UWCSVError('CSV_ZIP_ENCRYPTED', 'Encrypted ZIP entries are forbidden.');
    if (compressed === 0xffffffff || expanded === 0xffffffff) throw new UWCSVError('CSV_ZIP64_UNSUPPORTED', 'ZIP64 entries are not supported.');
    const nameStart = offset + 46;
    const nameEnd = nameStart + nameLength;
    const entryEnd = nameEnd + extraLength + commentLength;
    if (entryEnd > centralOffset + centralSize) throw new UWCSVError('CSV_ZIP_INVALID', 'ZIP central entry is truncated.');
    let name: string;
    try {
    ...
    if (names.has(name)) throw new UWCSVError('CSV_ZIP_DUPLICATE', `ZIP repeats ${name}.`);
    names.add(name);
    const unixMode = external >>> 16;
    if ((unixMode & 0xf000) === 0xa000) throw new UWCSVError('CSV_ZIP_SYMLINK', 'ZIP symlinks are forbidden.');
    total += expanded;
    if (total > maxExpanded) throw new UWCSVError('CSV_SIZE_LIMIT', `ZIP expands beyond ${maxExpanded} bytes.`);
    if (expanded > compressed * maxRatio + 1_048_576) throw new UWCSVError('CSV_ZIP_RATIO_LIMIT', `${name} exceeds compression ratio ${maxRatio}.`);
    offset = entryEnd;
  }
  if (offset !== centralOffset + centralSize) throw new UWCSVError('CSV_ZIP_INVALID', 'ZIP central directory size is inconsistent.');
}
