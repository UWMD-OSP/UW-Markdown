...
describe('calculation context transport', () => {
  it('preserves exact paths, variants, zeros, nulls and other JSON scalars', () => {
    const input = JSON.parse(
      '{"sectionVariants":{"dcf":"retail"},"overrides":{"dcf.annual_cash_flows@Y3.noi":0,"property.total_units":null,"label":"x","flag":false,"__proto__":17}}'
    );
  ...
  it('accepts empty contexts and null-prototype JSON objects', () => {
    ...
    expect(parseCalculationContext({ overrides: {}, sectionVariants: {} })).toEqual({
      overrides: {},
      sectionVariants: {},
    });
  });
  it.each([
    ...
    3,
    'x',
    Object.create({ overrides: {} }),
    { locale: 'en-US' },
    { overrides: null },
    { overrides: [] },
    { overrides: { x: [] } },
    { overrides: { x: {} } },
    { overrides: { x: Number.POSITIVE_INFINITY } },
