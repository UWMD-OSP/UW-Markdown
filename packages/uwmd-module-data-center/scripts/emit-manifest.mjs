...
const { DATA_CENTER_MODULE } = await import(pathToFileURL(join(root, 'dist', 'index.js')).href);

writeFileSync(
  join(root, 'dist', 'manifest.json'),
  `${JSON.stringify(DATA_CENTER_MODULE, null, 2)}\n`,
  'utf8',
);
console.log(`@uwmd/module-data-center: emitted dist/manifest.json (${DATA_CENTER_MODULE.id}@${DATA_CENTER_MODULE.version})`);
