...
Status: completed 2026-09-12 in implementation commit `f2204bd`, submitted as [PR #179](https://github.com/UWMD-OSP/UW-Markdown/pull/179).
...
- [x] Implement the reconciled contract across source, normative text, schemas,
  exports and fixtures; committed as `f2204bd`.
- [x] Pass all deterministic gates and submit dependent PR #179, based on #178.
- [x] Archive the completed contract and clear the active task matrix.

Verification: 1,782 workspace tests, 426 default conformance checks, 76 declarative
cases, build, test typechecking, 25 schemas, lint, lockfile/package/version/index
checks, and docs build. The corrected CommonJS imports in the schema test pass
both runtime and TypeScript checks. Existing calculation digests are unchanged;
two receipt protocol labels advance to 2.8.0. No package publication or merge.

Excel emission and refinement perturbation of selector expressions remain
explicitly unsupported. Relative quarter/month aliases, calendar conversion,
speculative leasing and module registration require separate scoped work.
