...
| [0036](./docs/rfcs/0036-waterfall-irr-hurdles.md) | ✅ IRR-hurdled waterfall tiers | **Shipped 2026-09-09** (protocol 2.6.0, §VIII.10 step 3; unreleased until the next cut). Takes up the `until_lp_irr` reservation from RFC 0035. The boundary is closed-form (the LP hurdle balance, `−xnpv(F,h)·(1+h)^t`), not the nested bisection 0035 §C sketched; laddered and combined hurdles follow; strictly-increasing hurdle ladders are `WF-01`. Corpus 385 → 396. |
