...
Current as of **2026-09-21**, following [release 2.13.0](https://github.com/UWMD-OSP/UW-Markdown/releases/tag/v2.13.0)
...
is **2.17.0**; the version streams are independent.
...
Released from the reviewed `v2.13.0` tag. The living detail is in the developer
wiki's build-status page
...
| **`@uwmd/lake` 0.2.1, lake schema 0.2** | The live-PostgreSQL fixes below. The package remains unpublished. |
...
| **Golden-deal release comparison** | Eight runnable cases pass in the release evidence: 532 Artifact B assertions accounted for, 526 passing, six documented baseline defects, and 20 refusal assertions passing. [De-identified evidence](docs/reviews/2026-09-21-golden-deal-release-comparison.md). This does not close RFC 0045-specific real-deal assembly evidence. |
...
|---|---|---|---|
| 1 | Real-deal DCF validation and extensions | Completed monthly evidence assessed; current-contract blockers identified | The [completed-corpus review](docs/reviews/2026-09-24-completed-golden-corpus-validation.md) replaces the all-annual blocker: monthly projections and dated metrics pass, while unchanged assembly refuses final-period timing and reserve-funded spending. The [accepted RFC 0062](docs/rfcs/0062-same-day-cash-flow-selection.md) reconciles same-day cash flows as Protocol 2.17.1 errata; its implementation is prepared but unreleased. Integration is complete at `e49eb43`. Private source/reference reconciliation is complete: 640/640 registered comparisons and 20/20 existing refusals pass, with a separate stale-workbook integrity refusal preserved. This includes stated-source checks and does not establish complete assembly parity. The [terminal-boundary owner brief](docs/reviews/2026-09-24-terminal-boundary-design.md) is prepared for decision: the demonstrated monthly cash is on disposition, at the final period's exclusive boundary, not after sale. It recommends optional monthly boundary admission; reserve-account and financing assembly remain separate. No new RFC or implementation is authorized. |
