...
The PCG64 stream now matches NumPy 1.26.4's compiled implementation under the
upstream default stream and srandom initialization: 11 seeds, 11,264 raw draws
and 176 doubles. Existing outputs are unchanged. The ordinary tests consume a
pinned independent oracle; `scripts/verify-pcg64-reference.py` reproduces it.
See the [verification record](../reviews/2026-09-12-pcg64-reference.md) for seed
mapping, upstream provenance and the scope of the comparison.

## Explicit period references (RFC 0041, Protocol 2.8.0)

`dcf.annual_cash_flows@Y3.net_operating_income` selects the row stating year 3,
independent of its position. Standard keyed NOI projections, lease-up schedules,
dated cash flows and stated waterfalls have corresponding registry entries.
Calendar months, quarters and dates retain absolute identities and never convert
implicitly to holding years. `periods.ts` owns canonicalization/inspection;
`period-path.ts` resolves contextual references. Generic `deepGet` is unchanged.
...
validated period variants and canonical period overrides; ordinary scalar
overrides are refused by this CLI. Refinement intervals remain unrounded:
compare them with reported calc endpoints at the existing quantization boundary,
as the core and CLI regression tests do.
