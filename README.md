...
- **v2 spec exploration** — locale negotiation, stochastic calcs, sensitivity tables, portfolio relationships. Each opens as an RFC under [`docs/rfcs/`](./docs/rfcs/). The signing chain shipped 2026-08-27: signed blocks ([RFC 0010](./docs/rfcs/0010-signed-blocks.md), protocol §V.11) and module signatures ([RFC 0002](./docs/rfcs/0002-module-signing.md), protocol §X.1), both on the optional [`@uwmd/signing`](./packages/uwmd-signing/) package; core stays crypto-free. A language-agnostic conformance driver ([RFC 0004](./docs/rfcs/0004-conformance-runner-v2.md), protocol §II.6a) shipped alongside, so an implementation in any language can self-certify against the same corpus — see [`conformance/runner/`](./conformance/runner/README.md). And [RFC 0006](./docs/rfcs/0006-hospitality-module.md) gave the module system its first real consumer — [`@uwmd/module-hospitality`](./packages/uwmd-module-hospitality/) — plus the runtime that actually executes what a module declares. [RFC 0003](./docs/rfcs/0003-module-asset-classes.md) then let a module declare its own **asset class** — the builtin ten stay closed, with a namespaced space beside them.
